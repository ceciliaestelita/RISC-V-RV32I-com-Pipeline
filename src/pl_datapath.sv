...
// =========================================================================
    ...
    always_comb begin
        case (ex_mem.funct3)
            3'b010: byte_enable = 4'b1111;                      // SW (palavra inteira)
            3'b001: begin                                       // SH (halfword)
                if (ex_mem.alu_result[1] == 1'b0)
                    byte_enable = 4'b0011;                      // halfword inferior (bits 1:0)
                else
                    byte_enable = 4'b1100;                      // halfword superior (bits 3:2)
            end
            ...
            default: byte_enable = 4'b0000;                     // segurança
        endcase
    end
    ...
    pl_dmem dmem (
        ...
        .ByteEnable(byte_enable),      //NOVO
        .addr      (ex_mem.alu_result[9:2]),
        .WriteData (ex_mem.write_data), //dado bruto (sem formatação)
        .ReadData  (dmem_rd)
    );
